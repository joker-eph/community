...
representation for shape inference, there is an independent design exploration
...
*   Initially the functional control flow operations are calling functions
    ...
    `tf_executor` dialect to be modeled in a graph.
...
`tf_executor.island` operation. These operands are always positioned after all
the explicit captures. If any operand is dead, the region is not executed and
dead values are immediately returned for every result.
...
alongside the value that is passed through the back-edge. No value is returned
